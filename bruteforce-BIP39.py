...
# https://gist.github.com/HeptaSean/f615f27128be4c1a7a66fe64d08e0b52
# NOTE: inclomplete seedphrase is entered above instead of through console input. Thus this works on online editors like https://thumbsdb.herokuapp.com/python/ or https://trinket.io/embed/python3/f03e855da4
...
def replace_unknown(seed: List[str],
                    unknown: List[int]) -> Iterable[List[str]]:
    if not unknown:
        yield seed
    else:
        for word in word_list:
            seed[unknown[0]] = word
            for seed in replace_unknown(seed, unknown[1:]):
                yield seed


def seed_to_bytes(seed: Iterable[str]) -> Tuple[bytes, int]:
    entropy = bytearray()
    bits = 0
    rest = 0
    for word in seed:
        number = word_list.index(word)
...
if __name__ == "__main__":
    ...
    #seed = sys.argv[1:]
    seed = testSeed.split()
#check if length of provided seed phrase is valid:
    if len(seed) not in [12, 15, 18, 21, 24]:
        print("Can only handle seed phrases of 12, 15, 18, 21, or 24 words.",
              file=sys.stderr)
        print(f"{len(seed)} words given.",
              file=sys.stderr)
        sys.exit(1)
#get index of all unknown words or placeholders:
    unknown = []
    for index, word in enumerate(seed):
        if word == "_":
            ...
            unknownWord = index;
            print(f"'{word}' is not in the BIP39 word list.",
                  file=sys.stderr)
            unknown.append(index)
    print(f"{len(unknown)} of {len(seed)} words unknown.",
          file=sys.stderr)
#brute-force seed phrase for every unknown word:
    print(" ")

    for seed in replace_unknown(seed, unknown):
        entropy, expected_checksum = seed_to_bytes(seed)
        if checksum(entropy) == expected_checksum:
          if(printAllWords):
            print(" ".join(seed))
            print("")

          else:
            print(seed[unknownWord])
